        ...
        <main class="account-shell">
            ...
            <div class="account-layout">
                <aside class="account-sidebar" aria-label="Tài khoản">
                    <a class="sidebar-link active" href="${pageContext.request.contextPath}/profile" title="Hồ sơ" aria-label="Hồ sơ">
                        <svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <a class="sidebar-link" href="${pageContext.request.contextPath}/redeem" title="Đổi voucher" aria-label="Đổi voucher">
                        <svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <section class="account-content">
                            ...
                            <label class="profile-avatar-picker" title="Đổi ảnh đại diện">
                                <c:choose>
                                    <c:when test="${not empty customer.displayAvatarUrl}">
                                        ...
                                        <img class="profile-avatar" src="${pageContext.request.contextPath}${customer.displayAvatarUrl}" alt="" onerror="this.hidden=true;this.previousElementSibling.hidden=false;">
                                    </c:when>
                                    <c:otherwise>
                    ...
                    <form class="profile-form ${editMode ? 'is-editing' : ''}" method="post" action="${pageContext.request.contextPath}/profile">
                        ...
                        <div class="profile-fields">
                            <label>Họ và tên<input name="fullName" value="<c:out value='${customer.fullName}'/>" ${editMode ? '' : 'readonly'} required>
                            </label>
                            <label>Số điện thoại<input name="phone" value="<c:out value='${customer.phone}'/>" ${editMode ? '' : 'readonly'} pattern="0[0-9]{9,10}">
                            </label>
                            <label>Giới tính<select name="gender" ${editMode ? '' : 'disabled'}>
                            ...
                            <label>Địa chỉ<input name="address" value="<c:out value='${customer.address}'/>" ${editMode ? '' : 'readonly'}>
                            </label>
                            <label>Username<input name="username" value="<c:out value='${customer.username}'/>" ${editMode ? '' : 'readonly'} required pattern="[A-Za-z0-9_]{4,50}">
                            </label>
                            <label>Email<input type="email" name="email" value="<c:out value='${customer.email}'/>" ${editMode ? '' : 'readonly'} required>
                            </label>
                        </div>
                    </form>
                </section>
